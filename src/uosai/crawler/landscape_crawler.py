# src/uosai/crawler/landscape_crawler.py
"""
조경학과 공지사항 크롤러
- URL: https://lauos.or.kr/notice
- 특징: 쿼리 파라미터 방식 (uid)
- 상세 URL: https://lauos.or.kr/notice?uid=123&mod=document
"""
...
def fetch_notice_html(post_id: int) -> Optional[str]:
    ...
    try:
        ...
        if r.status_code != 200:
            log(f"❌ [{DEPARTMENT}] 상세 요청 실패 uid={post_id}, status={r.status_code}")
            return None
        return r.text
    except Exception as e:
        log(f"❌ [{DEPARTMENT}] 요청 실패 uid={post_id}: {e}")
        return None
...
def parse_notice_fields(html: str, post_id: int) -> Optional[dict]:
    ...
    if not title:
        log(f"[{DEPARTMENT}] uid={post_id}: 제목 없음 (게시물 없거나 삭제됨)")
        return None

    # 본문 추출
    content_text = ""
    content_el = soup.select_one('div.kboard-content')
    if not content_el:
        content_el = soup.select_one('div.content-view')
    if content_el:
    ...
    # 날짜/조회수 추출 (div.detail-value 배열에서 인덱스로 파싱)
    date_text = ""
    view_count = 0
...
def process_one(post_id: int) -> str:
    ...
    if not html:
        log(f"⚠️ [{DEPARTMENT}] uid={post_id}: HTML 로드 실패 → 스킵")
        return "skipped_error"

    # 2) 파싱
    parsed = parse_notice_fields(html, post_id)
    if not parsed:
        log(f"[{DEPARTMENT}] uid={post_id}: 게시물 없음")
        return "not_found"
    ...
    if prev_dt:
        if prev_dt == curr_dt:
            # 날짜 동일 → 조회수만 업데이트
            log(f"[{DEPARTMENT}] uid={post_id} 이미 존재 → 조회수만 업데이트")
            update_view_count(CATEGORY, normalized_post_number, view_count)
            log(f"✅ [{DEPARTMENT}] 조회수 업데이트: uid={post_id}, viewCount={view_count}")
            return "stored"
        else:
            log(f"[{DEPARTMENT}] uid={post_id} 날짜 변경 {prev_dt} → {curr_dt}, 업데이트 진행")
    ...
    if not imgs:
        log(f"↳ [{DEPARTMENT}] uid={post_id}: 이미지 캡처 실패 → 스킵")
        return "skipped_error"

    # 8) 텍스트 + 이미지 요약
    summary = summarize_with_text_and_images(html_text, imgs)
    if not summary:
        log(f"↳ [{DEPARTMENT}] uid={post_id}: 요약 실패 → 스킵")
        return "skipped_error"
    ...
    try:
        upsert_notice(row)
        log(f"✅ [{DEPARTMENT}] 저장 완료: uid={post_id}, title={title}, posted_date={posted_date}")
        return "stored"
